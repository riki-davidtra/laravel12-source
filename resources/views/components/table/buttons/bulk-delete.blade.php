@props(['url', 'label' => 'Delete Selected', 'formId' => 'bulkDeleteForm', 'hiddenContainerId' => 'hiddenIds'])

<div x-data="{ isModalOpen: false }" id="{{ $formId }}" class="hidden">
...
@push('scripts')
    <script>
        ...
        function toggleBulkDeleteBtn() {
            ...
            if (selected) {
                $('#bulkDeleteForm').removeClass('hidden');
            } else {
                $('#bulkDeleteForm').addClass('hidden');
            }
        }
        ...
        $('#bulkDeleteForm').on('submit', function(e) {
            ...
            $('.row-checkbox:checked').each(function() {
                const id = $(this).val();
                $('#hiddenIds').append(`<input type="hidden" name="ids[]" value="${id}">`);
            });
        });
    </script>
@endpush
