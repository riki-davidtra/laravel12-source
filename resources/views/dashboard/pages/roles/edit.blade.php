...
@section('content')
    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        <x-breadcrumb title="Edit Role" :items="[['name' => 'Dashboard', 'url' => route('dashboard')], ['name' => 'Roles', 'url' => route('roles.index')], ['name' => 'Edit Role']]" />
                ...
                <div class="p-5 border-t border-gray-100 dark:border-gray-800 sm:p-6">
                    <form action="{{ route('roles.update', $role->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 spcace-y-4">
                            <x-form.select name="guard_name" label="Guard Name" :options="['web' => 'web', 'api' => 'api']" :selected="$guard" disabled />
                            <x-form.input name="name" label="Role" placeholder="Enter role" value="{{ old('name', $role->name) }}" />
                        </div>
